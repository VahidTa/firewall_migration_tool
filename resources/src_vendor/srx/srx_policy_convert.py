...
def srx_policy(file: str, vendor: str):
    ...
    try:
        security_cfg = dict_formatted["rpc-reply"]["multi-routing-engine-results"]["multi-routing-engine-item"][
            "security-policies"
        ]["security-context"]
    except Exception:
    ...
    for main_index in range(len(security_cfg)):
        ...
        for sub_index in range(len(policy)):
            ...
            if isinstance(policy, (list)):
                ...
                policy_destination_address = policy[sub_index]["policy-information"]["destination-addresses"][
                    "destination-address"
                ]
                ...
                policy_destination_address = policy["policy-information"]["destination-addresses"][
                    "destination-address"
                ]
                ...
                if isinstance(policy_application, (list)):
                    for inc_index in range(len(policy_application)):
                        ...
                        if vendor == "chpoint" and policy_app[:1].isdigit():
                ...
                policy_log = "log" in list(policy["policy-information"]["policy-action"].keys())
