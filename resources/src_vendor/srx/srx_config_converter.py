        ...
        for index in range(len(address_books)):
            ...
            if check:
                ...
                try:
                    ...
                    logger.warning(
                        f"source SRX config error. 'address' object does not exist for: {address_books[index]}. skipping ..."
                    )
                    continue
            ...
            for index in range(len(address_set_books)):
                ...
                if not check:
                    try:
                        ...
                        logger.warning(
                            f"source SRX config error. 'address-book-set' object does not exist for: {address_set_books[index]} . skipping ..."
                        )
                        continue
