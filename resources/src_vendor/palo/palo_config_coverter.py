        ...
        for index in range(len(applications_list)):
            ...
            if self.vendor == "forti":
                ...
                if "," in destination_port:
                    ...
                    logger.warn(
                        f"Skipping Service {application_name} since it has multiple ports set, which checkpoint can't handle. Please add it manually afterwards"
                    )
                    continue
                chpoint.service(
                    ServiceData(
    ...
    def service_set(self):
        ...
        for index in range(len(applications_list)):
            ...
            if self.vendor == "forti":
                ...
                srx.service_set(ServiceSetData(app_set_name=app_set_name, app_set_desc=app_set_desc, app_list=app_list))
            elif self.vendor == "chpoint":
                chpoint.service_set(
                    ServiceSetData(app_set_name=app_set_name, app_set_desc=app_set_desc, app_list=app_list)
                )
