...
class CHPoint_DST:
    ...
    def address(*args):
        ...
        if '/32' in address_ip:
            ...
            with open(the_path, 'a') as f:
                if os.path.getsize(the_path) == 0:
                    f.write(f'name,ip-address,set-if-exists,ignore-warnings,ignore-errors,comments\n')
                if address_desc:
            ...
            with open(the_path, 'a') as f:
                if os.path.getsize(the_path) == 0:
                    f.write(f'name,subnet,subnet-mask,set-if-exists,ignore-warnings,ignore-errors,comments\n')
                if address_desc:
    ...
    def address_set(*args):
        ...
        with open(the_path, 'a') as f:
            if os.path.getsize(the_path) == 0:
                f.write(f'name,ignore-warnings,ignore-errors,comments\n')
            if address_set_desc:
        ...
        with open(the_path, 'a') as f:
            if os.path.getsize(the_path) == 0:
                f.write(f'name,members.add,ignore-warnings,ignore-errors\n')
    ...
    def policy(*args):
        ...
        with open(the_path, 'a') as output:
            ...
            if policy_log:
                output.write(f'mgmt_cli add access-rule layer "Network" position "{position}" name "{policy_name}" action "{policy_action}" {policy_src_address} {policy_dst_address} {policy_app} enabled {policy_state} comments "original rule order {policy_id}" track Log -s "sid.txt"\n\n')
                
            else:
                output.write(f'mgmt_cli add access-rule layer "Network" position "{position}" name "{policy_name}" action "{policy_action}" {policy_src_address} {policy_dst_address} {policy_app} enabled {policy_state} comments "original rule order{policy_id}" -s "sid.txt"\n\n')
